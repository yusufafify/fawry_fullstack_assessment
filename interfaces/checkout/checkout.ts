...
import { Expirable, Shippable } from "../product/types";

export class ShippingService {
  ship(items: Shippable[]) {
...
export class Checkout {
  checkout(customer: Customer, cart: Cart) {
    ...
    for (const { product, quantity } of cart.getCartItems()) {
      if (this.checkForProductExpiary(product)) {
        throw new Error(
          `Error: ${product.getName()} is expired, please remove it from the cart\n`
        );
      }
      if (this.checkForProductAvailability(product, quantity)) {
        throw new Error(
          `Error: ${product.getName()} is out of stock during checkout\n`
        );
      }
    }
    ...
    for (const { product, quantity } of cart.getCartItems()) {
      ...
      if ("getWeight" in product) {
        for (let i = 0; i < quantity; i++) {
          toShip.push(product as unknown as Shippable);
        }
      }
    }
    ...
    cart
      ...
      .forEach(({ product, quantity }) =>
        product.reduceQuantityInStock(quantity)
      );
  ...
  checkForProductAvailability(product: Product, quantity: number): boolean {
    return quantity > product.getQuantityInStock();
  }

  checkForProductExpiary(product: Product) {
    return "isExpired" in product && (product as Expirable).isExpired();
  }
}
